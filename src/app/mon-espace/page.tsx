...
import {
  Ticket, Clock, CheckCircle2, XCircle, AlertCircle, LogOut,
  Download, MessageCircle, MapPin, QrCode, Search, ChevronRight,
  User, Phone, Calendar
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 font-sans">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-white/60 text-xs">{user?.emailAddresses[0]?.emailAddress}</p>
              </div>
              <div className="w-9 h-9 rounded-full bg-cnt-yellow/20 border-2 border-cnt-yellow flex items-center justify-center">
                <User className="w-4 h-4 text-cnt-yellow" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total billets', value: stats.total, color: 'from-cnt-blue to-blue-700', icon: <Ticket className="w-5 h-5" /> },
            { label: 'Valides', value: stats.valides, color: 'from-emerald-600 to-emerald-700', icon: <CheckCircle2 className="w-5 h-5" /> },
            { label: 'Utilisés', value: stats.utilises, color: 'from-gray-600 to-gray-700', icon: <Clock className="w-5 h-5" /> },
            { label: 'Annulés', value: stats.annules, color: 'from-red-500 to-red-700', icon: <XCircle className="w-5 h-5" /> },
          ].map((s) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          >
            ...
            <p className="text-xs opacity-80 mt-0.5">Réserver votre prochain trajet</p>
          </button>
          <button
          ...
          >
            <Image src="/icons/bus_tracking.png" alt="Suivi" width={24} height={24} className="w-6 h-6 mb-2 object-contain group-hover:scale-110 transition-transform" unoptimized />
            <p className="font-semibold text-gray-800">Suivre mon bus</p>
            <p className="text-xs text-gray-500 mt-0.5">Position GPS en direct</p>
          </button>
          <button
            onClick={() => router.push(`/api/tickets/verify?ref=${tickets[0]?.ref}`)}
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-0.5">Scanner ou saisir une référence</p>
          </button>
        </div>
          ...
          {filteredTickets.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400 font-medium">
                {tickets.length === 0 ? 'Aucun billet acheté pour le moment' : 'Aucun résultat pour cette recherche'}
              </p>
              {tickets.length === 0 && (
                <Button onClick={() => router.push('/')} className="mt-4 bg-cnt-green hover:bg-emerald-700 text-white rounded-xl">
            ...
            <div className="divide-y divide-gray-50">
              {filteredTickets.map((ticket) => (
                <div
                  ...
                  onClick={() => router.push(`/mon-espace/ticket/${ticket.ref}`)}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        <span className="text-xs text-gray-400 capitalize">{ticket.classe}</span>
                      </div>
